...
termAssignment _ category children
  = case (category, children) of
    ...
    (Modifier Unless, [lhs, rhs]) -> Just $ S.If (Term (In (setCategory (extract rhs) Negate) (S.Negate rhs))) [lhs]
    (Unless, expr : rest) -> Just $ S.If (Term (In (setCategory (extract expr) Negate) (S.Negate expr))) rest
    (Modifier Until, [ lhs, rhs ]) -> Just $ S.While (Term (In (setCategory (extract rhs) Negate) (S.Negate rhs))) [lhs]
    (Until, expr : rest) -> Just $ S.While (Term (In (setCategory (extract expr) Negate) (S.Negate expr))) rest
