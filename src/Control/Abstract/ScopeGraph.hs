...
putDeclarationSpan ::
  forall address sig m term value.
  ( Ord address,
    Has (State (ScopeGraph address)) sig m
  ) =>
  Declaration ->
...
insertExportEdge ::
  (Has (Reader (CurrentScope scopeAddress)) sig m, Has (State (ScopeGraph scopeAddress)) sig m, Ord scopeAddress) =>
  scopeAddress ->
...
runScopeError ::
  Evaluator term address value (Either.ResumableC (BaseError (ScopeError address)) m) a ->
