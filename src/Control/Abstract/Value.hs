...
data Function term address value (m :: * -> *) k
  =  Function Name [Name] term address (ValueRef address value -> k) -- ^ A function is parameterized by its name, parameter names, body, parent scope, and returns a ValueRef.
  | BuiltIn address BuiltIn (value -> k) -- ^ A built-in is parameterized by its parent scope, BuiltIn type, and returns a value.
  | Call value [value] (ValueRef address value -> k) -- ^ A Call takes a set of values as parameters and returns a ValueRef.
  deriving (Functor)
