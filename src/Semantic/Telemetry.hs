...
instance Effect Telemetry where
  handleState c dist (WriteStat stat) k = Request (WriteStat stat) (\result -> dist (pure result <$ c) k)
  handleState c dist (WriteLog level message pairs) k = Request (WriteLog level message pairs) (\result -> dist (pure result <$ c) k)
