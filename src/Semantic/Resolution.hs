...
instance Effect Resolution where
  handleState c dist (NodeJSResolution path key paths) k = Request (NodeJSResolution path key paths) (\result -> dist (pure result <$ c) k)
  handleState c dist NoResolution k = Request NoResolution (\result -> dist (pure result <$ c) k)
