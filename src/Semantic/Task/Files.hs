...
data Source blob where
  ...
  FromJSON :: Handle 'IO.ReadMode -> Source [Blob]
...
instance (Has (Error SomeException) sig m, MonadFail m, MonadIO m) => Algebra (Files :+: sig) (FilesC m) where
  ...
  alg (L op) = case op of
    ...
    Read (FromJSON handle) k -> readBlobsFromJSON handle >>= k
...
data FilesArg
  ...
  | FilesFromJSON (Handle 'IO.ReadMode)
...
readBlobs (FilesFromJSON handle) = send (Read (FromJSON handle) pure)
