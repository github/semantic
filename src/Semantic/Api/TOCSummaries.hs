  ...
  where
    go blobPair = decoratingDiffWith summarizeDiffParsers decorateTerm (pure . uncurry toFile . foldr combine ([], []) . summarizeDiff) blobPair
      ...
      where path = T.pack $ pathKeyForBlobPair blobPair
            ...
            combine :: Either ErrorSummary TOCSummary -> ([TOCSummaryChange], [TOCSummaryError]) -> ([TOCSummaryChange], [TOCSummaryError])
            combine (Right TOCSummary{..}) (changes, errors) = ((defMessage & P.category .~ kind & P.term .~ ident & P.maybe'span .~ (converting #? span) & P.changeType .~ toChangeType change) : changes, errors)

            combine (Left ErrorSummary{..}) (changes, errors) = (changes, (defMessage & P.error .~ message & P.maybe'span .~ converting #? span) : errors)
