...
alignDiff :: HasField fields Range => Both (Source Char) -> SyntaxDiff leaf (Record fields) -> [Join These (SplitDiff leaf fields)]
...
alignPatch :: forall fields leaf. HasField fields Range => Both (Source Char) -> Patch (SyntaxTerm leaf (Record fields)) -> [Join These (SplitDiff leaf fields)]
alignPatch sources patch = case patch of
  ...
  where getRange = characterRange . extract
        alignSyntax' :: (forall a. Identity a -> Join These a) -> Source Char -> SyntaxTerm leaf (Record fields) -> [Join These (SyntaxTerm leaf (Record fields))]
...
alignSyntax :: (Applicative f, HasField fields Range) => (forall a. f a -> Join These a) -> (SyntaxTermF leaf (Record fields) term -> term) -> (term -> Range) -> f (Source Char) -> SyntaxTermF leaf (f (Record fields)) [Join These term] -> [Join These term]
