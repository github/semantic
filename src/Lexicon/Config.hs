...
import           Prologue
import           Lexicon.Env
import           Lexicon.Telemetry
import qualified Lexicon.Telemetry.Error as Error
import qualified Lexicon.Telemetry.Stat as Stat
...
data Config
  = Config
  { configAppName                :: String               -- ^ Application name ("lexicon")
...
data Options
  ...
  , optionsFailOnWarning    :: Flag FailOnWarning    -- ^ Should lexicon fail fast on assignment warnings (for testing)
  , optionsFailOnParseError :: Flag FailOnParseError -- ^ Should lexicon fail fast on tree-sitter parser errors (for testing)
  }
...
defaultConfig options@Options{..} = do
  ...
  assignTimeout <- envLookupNum 4000 "LEXICON_ASSIGNMENT_TIMEOUT"
  pure Config
    { configAppName = "lexicon"
