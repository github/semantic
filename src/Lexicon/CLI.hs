...
import           Prologue
import           Lexicon.Api hiding (File)
import qualified Lexicon.AST as AST
import           Lexicon.Config
import qualified Lexicon.Graph as Graph
import qualified Lexicon.Task as Task
import           Lexicon.Task.Files
import           Lexicon.Telemetry
import qualified Lexicon.Telemetry.Log as Log
import           Lexicon.Version
...
arguments = info (version <*> helper <*> ((,) <$> optionsParser <*> argumentsParser)) description
  where
    ...
    versionString = "lexicon version " <> buildVersion <> " (" <> buildSHA <> ")"
    description = fullDesc <> header "lexicon -- Semantic (syntax-aware) diffs, program analysis toolkit"
