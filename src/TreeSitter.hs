...
parseRubyToAST :: Source -> IO (Source, A.Rose (Record '[Ruby.Grammar, Range]))
parseRubyToAST source = do
  ...
  ts_document_free document
  pure (source, ast)
  where toAST :: Node -> IO (A.RoseF (Record '[Ruby.Grammar, Range]) Node)
        toAST node@Node{..} = do
          let count = fromIntegral nodeChildCount
          children <- allocaArray count $ \ childNodesPtr -> do
            _ <- with nodeTSNode (\ nodePtr -> ts_node_copy_child_nodes nullPtr nodePtr childNodesPtr (fromIntegral count))
            peekArray count childNodesPtr
          pure $ A.RoseF (toEnum (fromIntegral nodeSymbol) :. nodeRange node :. Nil) children
