...
instance ToMarkup a => ToMarkup (Renderable (Bool, Int, Line a)) where
  toMarkup (Renderable (_, _, Line [])) = td mempty ! A.class_ (stringValue "blob-num blob-num-empty empty-cell") <> td mempty ! A.class_ (stringValue "blob-code blob-code-empty empty-cell") <> string "\n"
