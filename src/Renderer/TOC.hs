...
instance CustomHasDeclaration Markdown.Section where
  customToDeclaration Blob{..} _ (Markdown.Section level (Term (In headingAnn headingF), _) _)
    = Just $ SectionDeclaration (maybe (getSource (getField headingAnn)) (getSource . sconcat) (nonEmpty (getField . termAnnotation . unTerm <$> toList headingF))) mempty blobLanguage level
