...
askCache :: (Effectful m, Member (InCacheEffectFor m) effects) => CachingAnalysis m effects (CacheFor m)
askCache = lift ask

localCache :: (Effectful m, Member (InCacheEffectFor m) effects) => (CacheFor m -> CacheFor m) -> CachingAnalysis m effects a -> CachingAnalysis m effects a
localCache f a = lift (local f (lower a))

asksCache :: (Functor (m effects), Effectful m, Member (InCacheEffectFor m) effects) => (CacheFor m -> a) -> CachingAnalysis m effects a
asksCache f = f <$> askCache

getsCache :: (Functor (m effects), Effectful m, Member (OutCacheEffectFor m) effects) => (CacheFor m -> a) -> CachingAnalysis m effects a
getsCache f = f <$> getCache

getCache :: (Effectful m, Member (OutCacheEffectFor m) effects) => CachingAnalysis m effects (CacheFor m)
getCache = lift get

putCache :: (Effectful m, Member (OutCacheEffectFor m) effects) => CacheFor m -> CachingAnalysis m effects ()
putCache = lift . put

modifyCache :: (Effectful m, Member (OutCacheEffectFor m) effects, Monad (m effects)) => (CacheFor m -> CacheFor m) -> CachingAnalysis m effects ()
