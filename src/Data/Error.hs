...
showExcerpt colourize Span{..} Blob{..}
  ...
  . showString (replicate (caretPaddingWidth + lineNumberDigits) ' ') . withSGRCode colourize [SetColor Foreground Vivid Green] (showString (replicate caretWidth '^')) . showChar '\n'
  where context = fold contextLines
        ...
        caretWidth | posLine spanStart == posLine spanEnd = max 1 (posColumn spanEnd - posColumn spanStart)
                   | otherwise                            = 1
