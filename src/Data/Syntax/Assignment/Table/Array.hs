module Data.Syntax.Assignment.Table.Array
( Table
, tableAddresses
, singleton
, fromListWith
, toList
, lookup
) where

import Control.Arrow ((&&&))
import Data.Array
import Data.Functor.Classes
import qualified Data.Set as Set
import Data.List.NonEmpty (NonEmpty(..))
import Data.Semigroup (Max(..), Min(..), sconcat)
import GHC.Stack
import Prelude hiding (lookup)

data Table i a = Table { tableAddressSet :: Set.Set i, tableBranches :: Array i (Maybe a) }
  deriving (Eq, Foldable, Functor, Show, Traversable)

tableAddresses :: Ix i => Table i a -> [i]
tableAddresses = Set.toList . tableAddressSet


singleton :: Ix i => i -> a -> Table i a
singleton i a = Table (Set.singleton i) (listArray (i, i) [Just a])

fromListWith :: (HasCallStack, Ix i) => (a -> a -> a) -> [(i, a)] -> Table i a
fromListWith _ [] = error "fromList: empty list of associations"
fromListWith with assocs@(a:as) = Table (Set.fromList (fst <$> assocs)) (accumArray merge Nothing (getMin mn, getMax mx) assocs)
  where (mn, mx) = sconcat ((Min &&& Max) . fst <$> a:|as)
        merge Nothing b = Just b
        merge (Just a) b = Just (with a b)

toList :: Ix i => Table i a -> [(i, a)]
toList Table{..} = Set.toList tableAddressSet >>= \ addr -> maybe [] (pure . (,) addr) (tableBranches ! addr)


lookup :: Ix i => i -> Table i a -> Maybe a
lookup i Table{..}
  | bounds tableBranches `inRange` i = tableBranches ! i
  | otherwise                        = Nothing


instance (Ix i, Show i) => Show1 (Table i) where
  liftShowsPrec spA slA d t = showsBinaryWith showsPrec (const (liftShowList spA slA)) "Table" d (tableAddresses t) (toList t)
