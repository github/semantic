...
data AssignmentF ast grammar a where
  ...
  Choose :: [grammar] -> Array grammar (Maybe (Assignment ast grammar a)) -> Maybe (Assignment ast grammar a) -> AssignmentF ast grammar a
...
symbol s = tracing (Choose [s] (array (s, s) [(s, Just location)]) Nothing) `Then` return
...
choice alternatives
  ...
  | otherwise    = tracing (Choose symbols (accumArray merge Nothing bounds choices) (wrap . tracing . Alt . toList <$> nonEmpty atEnd)) `Then` return
  where (symbols, choices, atEnd) = foldMap toChoices alternatives
        ...
        toChoices rule = case rule of
          Tracing _ (Choose s c a) `Then` continue -> (s, ((id &&& (c !)) <$> s) >>= \ (sym, a) -> (,) sym . (>>= continue) <$> toList a, toList ((>>= continue) <$> a))
  ...
  where go :: Assignment ast grammar result -> State ast grammar -> Either (Error (Either String grammar)) (result, State ast grammar)
        ...
        run t yield initialState = expectedSymbols `seq` state `seq` maybe (anywhere Nothing) atNode (listToMaybe stateNodes)
          where atNode (node :< f) = case runTracing t of
                  ...
                  Choose _ choices _ | bounds choices `inRange` nodeSymbol node, Just choice <- choices ! nodeSymbol node -> go choice state >>= uncurry yield
                ...
                anywhere node = case runTracing t of
                  ...
                  Choose _ _ (Just atEnd) | Nothing <- node -> go atEnd state >>= uncurry yield
...
instance (Ix grammar, Show grammar, Show (ast (AST ast grammar))) => Show1 (AssignmentF ast grammar) where
  liftShowsPrec sp sl d a = case a of
    ...
    Choose symbols choices atEnd -> showsTernaryWith showsPrec (const (liftShowList showChild showChildren)) (liftShowsPrec showChild showChildren) "Choose" d symbols (assocs choices >>= \ (sym, a) -> (,) sym <$> toList a) atEnd
