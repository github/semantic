...
--   EnvironmentFor :: (Environment (LocationFor v) v -> Environment (LocationFor v) v) -> EvalEnv v -> EvalEnv v
--
...
instance (Recursive t
  ...
  eval (x:xs) = do
    ...
    env <- get @(EnvironmentFor v)       -- Get the global environment after evaluation since
