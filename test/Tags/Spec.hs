...
spec :: TestTree
spec = testGroup "Tags.Spec"
  [ testCase "produces tags for functions with docs" $ do
  ...
  , testCase "produces tags for methods" $ do
  ...
  , testCase "produces tags for calls" $ do
      (blob, tree) <- parseTestFile goParser "test/fixtures/go/tags/simple_functions.go"
      runTagging blob ["Call"] tree @?=
        [ Tag "Hi" "Call" (Span (Pos 7 2) (Pos 7 6)) ["Function", "Context", "Statements"] (Just "Hi()") Nothing]

  , testGroup "javascript/typescript"
      [ testCase "produces tags for functions with docs" $ do
      ...
      , testCase "produces tags for classes" $ do
      ...
      , testCase "produces tags for modules" $ do
  ...
  , testGroup "python"
    [ testCase "produces tags for functions" $ do
    ...
    , testCase "produces tags for multi-line functions" $ do
  ...
  , testGroup "ruby"
    ...
    , testCase "produces tags for sends" $ do
        (blob, tree) <- parseTestFile rubyParser "test/fixtures/ruby/tags/simple_method.rb"
        runTagging blob ["Send"] tree @?=
    ...
    , testCase "produces tags for methods with docs" $ do
    ...
    , testCase "produces tags for methods and classes with docs" $ do
