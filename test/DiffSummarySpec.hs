...
testDiff :: SyntaxDiff Text (Record '[Category, Range])
  ...
  describe "DiffInfo" $ do
    ...
    prop "generates one LeafInfo for each child in an arbitrary branch patch" $
      \a -> let
        ...
        extractDiffLeaves :: SyntaxTerm Text (Record '[Category, Range]) -> [ SyntaxTerm Text (Record '[Category, Range]) ]
...
isIndexedOrFixed :: Patch (SyntaxTerm a annotation) -> Bool
