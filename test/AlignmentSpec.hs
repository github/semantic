  ...
  describe "alignDiff" $ do
    it "aligns identical branches on a single line" $
      ...
      align sources (pure (info 0 7) `copy` Indexed [ pure (info 2 5) `copy` Leaf "foo" ]) `shouldBe` prettyDiff sources
        [ Join (These (wrap $ info 0 7 :< [ wrap $ info 2 5 :< [] ])
                      (wrap $ info 0 7 :< [ wrap $ info 2 5 :< [] ])) ]

    it "aligns identical branches spanning multiple lines" $
      let sources = both (Source.fromText "[\nfoo\n]") (Source.fromText "[\nfoo\n]") in
      align sources (pure (info 0 7) `copy` Indexed [ pure (info 2 5) `copy` Leaf "foo" ]) `shouldBe` prettyDiff sources
        [ Join (These (wrap $ info 0 2 :< [])
                      (wrap $ info 0 2 :< []))
        , Join (These (wrap $ info 2 6 :< [ wrap $ info 2 5 :< [] ])
                      (wrap $ info 2 6 :< [ wrap $ info 2 5 :< [] ]))
        , Join (These (wrap $ info 6 7 :< [])
                      (wrap $ info 6 7 :< []))
        ]
    ...
    it "aligns reformatted branches" $
      ...
      align sources (pure (info 0 7) `copy` Indexed [ pure (info 2 5) `copy` Leaf "foo" ]) `shouldBe` prettyDiff sources
        [ Join (That  (wrap $ info 0 2 :< []))
        , Join (These (wrap $ info 0 7 :< [ wrap $ info 2 5 :< [] ])
                      (wrap $ info 2 6 :< [ wrap $ info 2 5 :< [] ]))
        , Join (That  (wrap $ info 6 7 :< []))
        ]
    ...
    it "aligns nodes following reformatted branches" $
      ...
      align sources (pure (info 0 12) `copy` Indexed [ pure (info 0 7) `copy` Indexed [ pure (info 2 5) `copy` Leaf "foo" ], pure (info 8 11) `copy` Leaf "bar" ]) `shouldBe` prettyDiff sources
        [ Join (That  (wrap $ info 0 2   :< [ wrap $ info 0 2  :< [] ]))
        , Join (These (wrap $ info 0 8   :< [ wrap $ info 0 7  :< [ wrap $ info 2 5 :< [] ] ])
                      (wrap $ info 2 6   :< [ wrap $ info 2 6  :< [ wrap $ info 2 5 :< [] ] ]))
        , Join (That  (wrap $ info 6 8   :< [ wrap $ info 6 7  :< [] ]))
        , Join (These (wrap $ info 8 12  :< [ wrap $ info 8 11 :< [] ])
                      (wrap $ info 8 12  :< [ wrap $ info 8 11 :< [] ]))
        , Join (These (wrap $ info 12 12 :< [])
                      (wrap $ info 12 12 :< []))
        ]
    ...
    it "aligns identical branches with multiple children on the same line" $
      ...
      align sources (pure (info 0 12) `copy` Indexed [ pure (info 2 5) `copy` Leaf "foo", pure (info 7 10) `copy` Leaf "bar" ]) `shouldBe` prettyDiff sources
        [ Join (runBothWith These (pure (wrap $ info 0 12 :< [ wrap $ info 2 5 :< [], wrap $ info 7 10 :< [] ])) ) ]

    it "aligns insertions" $
      let sources = both (Source.fromText "a") (Source.fromText "a\nb") in
      align sources (both (info 0 1) (info 0 3) `copy` Indexed [ pure (info 0 1) `copy` Leaf "a", inserting (Term (info 2 3 :< Leaf "b")) ]) `shouldBe` prettyDiff sources
        [ Join (These (wrap $ info 0 1 :< [ wrap $ info 0 1 :< [] ])
                      (wrap $ info 0 2 :< [ wrap $ info 0 1 :< [] ]))
        , Join (That  (wrap $ info 2 3 :< [ pure (SplitInsert (Term (info 2 3 :< []))) ]))
        ]

    it "aligns total insertions" $
      let sources = both (Source.fromText "") (Source.fromText "a") in
      align sources (inserting (Term (info 0 1 :< Leaf "a"))) `shouldBe` prettyDiff sources
        [ Join (That (pure (SplitInsert (Term (info 0 1 :< []))))) ]

    it "aligns insertions into empty branches" $
      let sources = both (Source.fromText "[ ]") (Source.fromText "[a]") in
      align sources (pure (info 0 3) `copy` Indexed [ inserting (Term (info 1 2 :< Leaf "a")) ]) `shouldBe` prettyDiff sources
        [ Join (That  (wrap $ info 0 3 :< [ pure (SplitInsert (Term (info 1 2 :< []))) ]))
        , Join (This  (wrap $ info 0 3 :< []))
        ]

    it "aligns symmetrically following insertions" $
      let sources = both (Source.fromText "a\nc") (Source.fromText "a\nb\nc") in
      align sources (both (info 0 3) (info 0 5) `copy` Indexed [ pure (info 0 1) `copy` Leaf "a", inserting (Term (info 2 3 :< Leaf "b")), both (info 2 3) (info 4 5) `copy` Leaf "c" ])
        `shouldBe` prettyDiff sources
        [ Join (These (wrap $ info 0 2 :< [ wrap $ info 0 1 :< [] ])
                      (wrap $ info 0 2 :< [ wrap $ info 0 1 :< [] ]))
        , Join (That  (wrap $ info 2 4 :< [ pure (SplitInsert (Term (info 2 3 :< []))) ]))
        , Join (These (wrap $ info 2 3 :< [ wrap $ info 2 3 :< [] ])
                      (wrap $ info 4 5 :< [ wrap $ info 4 5 :< [] ]))
        ]
    ...
    it "symmetrical nodes force the alignment of asymmetrical nodes on both sides" $
      ...
      align sources (pure (info 0 8) `copy` Indexed [ deleting (Term (info 2 3 :< Leaf "a")), both (info 5 6) (info 2 3) `copy` Leaf "b", inserting (Term (info 5 6 :< Leaf "c")) ]) `shouldBe` prettyDiff sources
        [ Join (These (wrap $ info 0 8 :< [ pure (SplitDelete (Term (info 2 3 :< []))), wrap $ info 5 6 :< [] ])
                      (wrap $ info 0 8 :< [ wrap $ info 2 3 :< [], pure (SplitInsert (Term (info 5 6 :< []))) ])) ]
    ...
    it "when one of two symmetrical nodes must be split, splits the latter" $
      ...
      align sources (both (info 0 8) (info 0 9) `copy` Indexed [ pure (info 2 3) `copy` Leaf "a", both (info 5 6) (info 6 7) `copy` Leaf "b" ]) `shouldBe` prettyDiff sources
        [ Join (These (wrap $ info 0 8 :< [ wrap $ info 2 3 :< [], wrap $ info 5 6 :< [] ])
                      (wrap $ info 0 4 :< [ wrap $ info 2 3 :< [] ]))
        , Join (That  (wrap $ info 4 8 :< [ wrap $ info 6 7 :< [] ]))
        , Join (That  (wrap $ info 8 9 :< []))
        ]

    it "aligns deletions before insertions" $
      let sources = both (Source.fromText "[ a ]") (Source.fromText "[ b ]") in
      align sources (pure (info 0 5) `copy` Indexed [ deleting (Term (info 2 3 :< Leaf "a")), inserting (Term (info 2 3 :< Leaf "b")) ]) `shouldBe` prettyDiff sources
        [ Join (This  (wrap $ info 0 5 :< [ pure (SplitDelete (Term (info 2 3 :< []))) ]))
        , Join (That  (wrap $ info 0 5 :< [ pure (SplitInsert (Term (info 2 3 :< []))) ]))
        ]
    ...
    it "aligns context-only lines symmetrically" $
      ...
      align sources (both (info 0 13) (info 0 12) `copy` Indexed [ pure (info 4 5) `copy` Leaf "a", both (info 10 11) (info 7 8) `copy` Leaf "b" ]) `shouldBe` prettyDiff sources
        [ Join (These (wrap $ info 0 2 :< [])
                      (wrap $ info 0 2 :< []))
        , Join (These (wrap $ info 2 6 :< [ wrap $ info 4 5 :< [] ])
                      (wrap $ info 2 9 :< [ wrap $ info 4 5 :< [], wrap $ info 7 8 :< [] ]))
        , Join (These (wrap $ info 6 8 :< [])
                      (wrap $ info 9 10 :< []))
        , Join (This  (wrap $ info 8 12 :< [ wrap $ info 10 11 :< [] ]))
        , Join (These (wrap $ info 12 13 :< [])
                      (wrap $ info 10 11 :< []))
        , Join (That  (wrap $ info 11 12 :< []))
        ]
    ...
    it "aligns asymmetrical nodes preceding their symmetrical siblings conservatively" $
      ...
      align sources (both (info 0 8) (info 0 9) `copy` Indexed [ inserting (Term (info 2 3 :< Leaf "a")), deleting (Term (info 2 3 :< Leaf "b")), both (info 5 6) (info 6 7) `copy` Leaf "c" ]) `shouldBe` prettyDiff sources
        [ Join (That  (wrap $ info 0 4 :< [ pure (SplitInsert (Term (info 2 3 :< []))) ]))
        , Join (These (wrap $ info 0 8 :< [ pure (SplitDelete (Term (info 2 3 :< []))), wrap $ info 5 6 :< [] ])
                      (wrap $ info 4 8 :< [ wrap $ info 6 7 :< [] ]))
        , Join (That  (wrap $ info 8 9 :< []))
        ]
    ...
    it "aligns symmetrical reformatted nodes" $
      ...
      align sources (pure (info 0 9) `copy` Indexed [ pure (info 0 1) `copy` Leaf "a", pure (info 2 7) `copy` Indexed [ pure (info 4 5) `copy` Leaf "b" ], pure (info 8 9) `copy` Leaf "c" ]) `shouldBe` prettyDiff sources
        [ Join (These (wrap $ info 0 8 :< [ wrap $ info 0 1 :< [], wrap $ info 2 7 :< [ wrap $ info 4 5 :< [] ] ])
                      (wrap $ info 0 4 :< [ wrap $ info 0 1 :< [], wrap $ info 2 4 :< [] ]))
        , Join (That  (wrap $ info 4 6 :< [ wrap $ info 4 6 :< [ wrap $ info 4 5 :< [] ] ]))
        , Join (That  (wrap $ info 6 8 :< [ wrap $ info 6 7 :< [] ]))
        , Join (These (wrap $ info 8 9 :< [ wrap $ info 8 9 :< [] ])
                      (wrap $ info 8 9 :< [ wrap $ info 8 9 :< [] ]))
        ]
