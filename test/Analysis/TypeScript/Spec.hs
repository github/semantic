...
spec = do
  describe "TypeScript" $ do
    it "qualified export from" $ do
      ...
      case moduleLookup "main6.ts" <$> res of
    ...
    it "imports with aliased symbols" $ do
      ...
      case moduleLookup "main.ts" <$> res of
        Right (Just (Module _ (scopeAndFrame, _))) -> do
    ...
    it "imports with qualified names" $ do
      ...
      case moduleLookup "main1.ts" <$> res of
        Right (Just (Module _ (scopeAndFrame, _))) -> do
    ...
    it "imports functions" $ do
      ...
      case moduleLookup "main4.ts" <$> res of
        Right (Just (Module _ (scopeAndFrame, value))) -> do
          SpecHelpers.lookupDeclaration "foo" scopeAndFrame heap scopeGraph `shouldSatisfy` isJust
    ...
    it "side effect only imports dont expose exports" $ do
      ...
      case moduleLookup "main3.ts" <$> res of
        Right (Just (Module _ (scopeAndFrame, value))) -> do
          SpecHelpers.lookupDeclaration "baz" scopeAndFrame heap scopeGraph `shouldBe` Nothing
          value `shouldBe` Unit
    ...
    it "fails exporting symbols not defined in the module" $ do
      ...
      res `shouldBe` Left (SomeError (inject @(BaseError (ScopeError Precise)) (BaseError (ModuleInfo (Path.absRel "bad-export.ts") "TypeScript" mempty) (Span (Pos 2 1) (Pos 2 28)) ImportReferenceError)))

    it "evaluates early return statements" $ do
      (scopeGraph, (heap, res)) <- evaluate ["early-return.ts"]
      case moduleLookup "early-return.ts" <$> res of
    ...
    it "evaluates sequence expressions" $ do
      ...
      case moduleLookup "sequence-expression.ts" <$> res of
        Right (Just (Module _ (scopeAndFrame, _))) ->
    ...
    it "evaluates await" $ do
      ...
      case moduleLookup "await.ts" <$> res of
        Right (Just (Module _ (scopeAndFrame, _))) -> do
    ...
    it "evaluates BOr statements" $ do
      ...
      case moduleLookup "bor.ts" <$> res of
    ...
    it "evaluates BAnd statements" $ do
      ...
      case moduleLookup "band.ts" <$> res of
    ...
    it "evaluates BXOr statements" $ do
      ...
      case moduleLookup "bxor.ts" <$> res of
        Right (Just (Module _ (_, value))) -> value `shouldBe` Concrete.Integer (Number.Integer 3)
        other                              -> expectationFailure (show other)

    it "evaluates LShift statements" $ do
      (_, (_, res)) <- evaluate ["lshift.ts"]
      case moduleLookup "lshift.ts" <$> res of
    ...
    it "evaluates RShift statements" $ do
      ...
      case moduleLookup "rshift.ts" <$> res of
        Right (Just (Module _ (_, value))) -> value `shouldBe` Concrete.Integer (Number.Integer 0)
        other                              -> expectationFailure (show other)

    it "evaluates Complement statements" $ do
      (_, (_, res)) <- evaluate ["complement.ts"]
      case moduleLookup "complement.ts" <$> res of
    ...
    it "uniquely tracks public fields for instances" $ do
      ...
      case moduleLookup "class1.ts" <$> res of
    ...
    it "member access of private field definition throws AccessControlError" $ do
      ...
      let expected = Left (SomeError (inject @TypeScriptEvalError (BaseError (ModuleInfo (Path.absRel "private_field_definition.ts") "TypeScript" mempty) (Span (Pos 4 1) (Pos 4 6)) (AccessControlError ("foo", ScopeGraph.Public) ("y", ScopeGraph.Private) (Concrete.Float (Decimal 2.0))))))
    ...
    it "member access of private static field definition throws AccessControlError" $ do
      ...
      let expected = Left (SomeError (inject @TypeScriptEvalError (BaseError (ModuleInfo (Path.absRel "private_static_field_definition.ts" )"TypeScript" mempty) (Span (Pos 3 1) (Pos 3 8)) (AccessControlError ("Adder", ScopeGraph.Public) ("z", ScopeGraph.Private) Unit))))
      res `shouldBe` expected
    ...
    it "member access of private methods throws AccessControlError" $ do
      ...
      let expected = Left (SomeError (inject @TypeScriptEvalError (BaseError (ModuleInfo (Path.absRel "private_method.ts" )"TypeScript" mempty) (Span (Pos 4 1) (Pos 4 16)) (AccessControlError ("foo", ScopeGraph.Public) ("private_add", ScopeGraph.Private) (Closure (PackageInfo "access_control" mempty) (ModuleInfo (Path.absRel "adder.ts") "TypeScript" mempty) (Just "private_add") Nothing [] (Right (TypeScript.Term (In (Loc (Range 146 148) (Span (Pos 7 27) (Pos 7 29))) (inject (StatementBlock []))))) (Precise 20) (Precise 18))))))
      res `shouldBe` expected
