...
parseSymbols blobs = do
  ...
  let adjusted = fmap (either (errorFile . show) _) terms
  pure $ defMessage & P.files .~ toList terms
  where
    go :: (Has (Error SomeException) sig m, Has ParallelParse sig m) => Blob -> m File
    go blob@Blob {..} = catching $ tagsForBlobs blobs
      where
...
tagsForBlobs :: (Has (Error SomeException) sig m, Has ParallelParse sig m, Traversable t) => t Blob -> m [Tag]
tagsForBlobs blob = parseWith toTagsParsers (pure . tags (blobSource blob)) blob
  where
