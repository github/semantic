...
spec = do
  ...
  describe "python" $ do
    it "produces tags for functions" $
      parseTestFile [] (Path.relFile "semantic/test/fixtures/python/tags/simple_functions.py")
    ...
    it "produces tags for functions with docs" $
      parseTestFile [] (Path.relFile "semantic/test/fixtures/python/tags/simple_function_with_docs.py")
        `shouldReturn` [ Tag "Foo" P.FUNCTION P.DEFINITION (Range 4 7) (OneIndexedSpan (Span (Pos 1 5) (Pos 1 8))) "def Foo(x):" (UTF16CodeUnitSpan (Span (Pos 0 4) (Pos 0 7)))
                       ]
    it "produces tags for classes" $
      parseTestFile [] (Path.relFile "semantic/test/fixtures/python/tags/class.py")
    ...
    it "produces tags for multi-line functions" $
      parseTestFile [P.FUNCTION] (Path.relFile "semantic/test/fixtures/python/tags/multiline.py")
        `shouldReturn` [ Tag "Foo" P.FUNCTION P.DEFINITION (Range 4 7) (OneIndexedSpan (Span (Pos 1 5) (Pos 1 8))) "def Foo(x," (UTF16CodeUnitSpan (Span (Pos 0 4) (Pos 0 7)))
                       ]
  describe "ruby" $ do
    it "produces tags for methods" $
