...
replaceLeavesWithWordBranches source term = replaceIn source 0 term
  where
    replaceIn source startIndex (info@(Info range lineRange categories) :< syntax) | substring <- substring range source, subrange <- offsetRange (negate startIndex) range = info :< case syntax of
